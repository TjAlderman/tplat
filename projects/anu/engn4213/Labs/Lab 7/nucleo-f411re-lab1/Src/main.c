...
int pulse;
int period;
uint8_t buffer_Tx[20]= "Please help u7287129";
...
int main(void)
{
  ...
  pulse = 0;
  HAL_UART_Receive_DMA(&huart2, buffer_Rx, 20);
  /* USER CODE END 2 */
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim2) { // callback to identify timer rollover and reactivate blinking
	period = 1;
	pulse = 0;
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_RESET);
}
void HAL_TIM_PWM_PulseFinishedCallback(TIM_HandleTypeDef *htim2) { // callback to identify end of PWM on and pause blinking
	pulse = 1;
